...
# 将bunch转化成TF-IDF权重矩阵
def makeTfidfSpace(bunch, vocabulary, stopList):
    tfidfSpace = Bunch(targetName=bunch.targetName, label=bunch.label, fileNames=bunch.fileNames,
                       contents=bunch.contents, tdm=[], vocabulary=vocabulary)
    '''
    ...
    if vocabulary is  None:
        vectorizer = TfidfVectorizer(stop_words=stopList, sublinear_tf=True, max_df=0.8)
    else:
        vectorizer = TfidfVectorizer(stop_words=stopList, vocabulary=vocabulary, sublinear_tf=True, max_df=0.8)
...
print("训练集构建训练集tf_idf")
tfidfSpace = makeTfidfSpace(bunch, None, stopList)  # 获取TF-IDF
print("训练集构建完成,正在写入")
...
写入测试集的tfidfSpace
'''
dictData = tfidfSpace.vocabulary
path = rootPath + "/test_word_bag/testSpace.dat"
bunch = readBunch(path)  # 读取bunch

print("测试构建训练集tf_idf")
tfidfSpace = makeTfidfSpace(bunch, tfidfSpace.vocabulary, stopList)  # 获取TF-IDF
print("测试构建完成正在写入")
